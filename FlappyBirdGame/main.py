...
def welcomeScreen():
    ...
    while True:
        for event in pygame.event.get():
            # if user click on cross button, closs the game
            if ((event.type == QUIT) or (event.type == KEYDOWN and event.type == K_ESCAPE)):
                pygame.quit()
                sys.exit()
                
            # if user click up or space key start the game
                ...
                pygame.display.update() #screen change
                FPSCLOCK.tick(FPS) #max fps will be 32
...
def mainGame():
    ...
    while True:
        ...
        #this function return true if you are crashed
        ...
        for pipe in upperPipes:
            ...
            if pipeMidPos <= playerMidPos < pipeMidPos+4: # taking band which when crossed inc. score
                score += 1
        ...
        # Add a new pipe when the first pipe is about to cross the left
...
def getRandomPipe():
    ...
    #1 or bottom and 1 rotated for top
...
if __name__ == "__main__":
    ...
    pygame.display.set_caption('FlappyBird by Rishabh')
